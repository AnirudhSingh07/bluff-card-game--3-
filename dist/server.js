...
const RECONNECT_GRACE_MS = 90000; // 90s to reconnect in-game
const LOBBY_DISCONNECT_GRACE_MS = 30000; // 30s to reconnect in lobby
...
/* =====================================================================
   HELPERS
   ===================================================================== */
const generatePlayerCode = () => crypto_1.default.randomBytes(4).toString("hex").toUpperCase();
const createDeck = () => {
    const deck = [];
...
const getPlayerIndex = (game, socketId) => game.players.findIndex((p) => p.id === socketId);
...
const checkWinner = (game) => {
    if (game.winner !== undefined)
        return true;
    if (game.potentialWinner !== undefined &&
...
app.prepare().then(() => {
    ...
    io.on("connection", (socket) => {
        ...
        /* -----------------------------------------------------------------
           CREATE GAME
           ----------------------------------------------------------------- */
        socket.on("create-game", ({ name, maxPlayers }) => guard(() => {
        ...
        /* -----------------------------------------------------------------
           JOIN GAME
           ----------------------------------------------------------------- */
        socket.on("join-game", ({ gameId, name }) => guard(() => {
        ...
        /* -----------------------------------------------------------------
           RECONNECT PLAYER
           ----------------------------------------------------------------- */
        socket.on("reconnect-player", ({ gameId, playerCode }) => guard(() => {
            const game = games.get(gameId);
            if (!game)
                return socket.emit("error", "Game not found");
        ...
        /* -----------------------------------------------------------------
           GET GAME STATE
           ----------------------------------------------------------------- */
        socket.on("get-game-state", ({ gameId }) => guard(() => {
            const game = games.get(gameId);
            if (game)
                socket.emit("game-updated", toClient(game));
        }));
        /* -----------------------------------------------------------------
           ...
           ----------------------------------------------------------------- */
        socket.on("start-game", ({ gameId }) => guard(() => {
            const game = games.get(gameId);
            if (!game)
                return socket.emit("error", "Game not found");
        ...
        /* -----------------------------------------------------------------
           PLAY CARDS
           ----------------------------------------------------------------- */
        socket.on("play-cards", ({ gameId, selected, count, type }) => guard(() => {
            const game = games.get(gameId);
            ...
            for (const card of selected) {
                ...
                if (idx === -1)
                    return socket.emit("error", "Card not in hand");
        ...
        socket.on("check", ({ gameId }) => guard(() => {
            ...
            const lied = game.lastClaim.cards.some((c) => c.slice(0, -1) !== game.lastClaim.type);
            if (lied) {
                claimer.hand.push(...game.pile);
                game.log.push(`${checker.name} challenged ${claimer.name} — ❌ LIED! ${claimer.name} takes ${game.pile.length} cards.`);
                game.turn = checkerIndex;
            }
            else {
                checker.hand.push(...game.pile);
                game.log.push(`${checker.name} challenged ${claimer.name} — ✅ HONEST! ${checker.name} takes ${game.pile.length} cards.`);
        ...
        /* -----------------------------------------------------------------
           SEND EMOTE
           ----------------------------------------------------------------- */
        socket.on("send-emote", ({ gameId, emoji }) => guard(() => {
            const game = games.get(gameId);
            if (!game)
                return;
            const playerIndex = getPlayerIndex(game, socket.id);
        ...
        /* -----------------------------------------------------------------
           REMATCH (host only)
           ----------------------------------------------------------------- */
        socket.on("rematch", ({ gameId }) => guard(() => {
            const game = games.get(gameId);
            if (!game)
                return socket.emit("error", "Game not found");
            if (game.hostSocketId !== socket.id)
            ...
            for (const [, game] of games) {
                ...
                if (game.status === "lobby") {
                    ...
                    setTimeout(() => {
                        ...
                        if (game.hostPlayerCode === player.playerCode &&
